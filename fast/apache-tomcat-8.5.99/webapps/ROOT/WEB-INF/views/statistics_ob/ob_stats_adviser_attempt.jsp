...
<div id="wrap">
	...
	<jsp:include page="../common/inc_header.jsp">
		...
		<jsp:param name="titleTxt" value="상담원별 통계(시도기준)"/>
	</jsp:include>
	<!-- .#header -->
        ...
        <div class="srchArea">
            <table class="tbl_line_view" summary="검색유형/모음기준/제외여부/검색일자/개인지정으로 구성됨">
                ...
                <tr>
                    ...
                    <td>
                        <div class="radioBox">
                            <input type="radio" name="ipt_radio1" id="ipt_radio1_1" class="ipt_radio" value="30min" checked>
                            <label for="ipt_radio1_1">30분 단위</label>
                            <input type="radio" name="ipt_radio1" id="ipt_radio1_2" class="ipt_radio" value="hourly">
                            <label for="ipt_radio1_2">시간 단위</label>
                            <input type="radio" name="ipt_radio1" id="ipt_radio1_3" class="ipt_radio" value="daily">
                            <label for="ipt_radio1_3">일 단위</label>
                            <input type="radio" name="ipt_radio1" id="ipt_radio1_4" class="ipt_radio" value="weekly">
                            <label for="ipt_radio1_4">요일 단위</label>
                            <input type="radio" name="ipt_radio1" id="ipt_radio1_5" class="ipt_radio" value="monthly">
                    ...
                    <td>
                        <div class="radioBox">
                            <input type="radio" name="ipt_radio2" id="ipt_radio2_1" class="ipt_radio" checked>
                            <label for="ipt_radio2_1">개인 | (시/일/월/요일)</label>
                            <input type="radio" name="ipt_radio2" id="ipt_radio2_2" class="ipt_radio">
                            <label for="ipt_radio2_2">(시/일/월/요일) | 개인</label>
                            <!-- [D] <label>의 for값과 <input>의 ID 값을 동일하게 작성해야 함 -->
                        </div>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <select id="multiple_select" class="select" multiple="multiple">
                            <option>전체</option>
                            <option>음성봇1 (상담사1)</option>
                            <option>음성봇2 (상담사1)</option>
                            <option>음성봇3 (상담사1)</option>
                            <option>음성봇4 (상담사1)</option>
                            <option>음성봇5 (상담사1)</option>
                            <option>음성봇6 (상담사2)</option>
                            <option>음성봇7 (상담사2)</option>
                        </select>
                    </td>
                </tr>
                </tbody>
...
jQuery.event.add(window,"load",function(){
	$(document).ready(function (){
        ...
        //jqGrid(data)
        /* var mydata = [
            {divide:'00:00', adviser:'음성봇1 (상담사1)', dbNum:'70',
			 ...
			 connect:'71.4%',
             rule01:'28.6%', rule02:'40.0%', rule03:'40.0%',
        ...
        var colName1 = ['상담원','구분','할당DB수','연결성공','통화중','결번','수신거부','계','목적성공','재통화예약','부재중','통화중끊김','통화거부','계','연결율','DB기준','연결기준','총통화시간','평균통화시간','총통화시간','평균통화시간'];
        var colName2 = ['구분','상담원','할당DB수','연결성공','통화중','결번','수신거부','계','목적성공','재통화예약','부재중','통화중끊김','통화거부','계','연결율','DB기준','연결기준','총통화시간','평균통화시간','총통화시간','평균통화시간'];
        var colModel1 =
        	[
				{name:'adviser', index:'adviser', width:70, align:'center'},
                {name:'divide', index:'divide', width:100, align:'center'},
        ...
        var sortname1 = 'divide';
        var sortname2 = 'adviser';

        var colName = colName2;
        var colModel = colModel2;
        var sortname = sortname2;

        function createJqGrid() {
        $("#jqGrid").jqGrid({
        	url: "${pageContext.request.contextPath}/getObStatsAdviserAttemptJQList",
            //data: mydata,
            datatype: "local",
            height: 'auto',
            rowNum: 30,
		...
		$("#jqGrid").jqGrid('setGroupHeaders', {
			...
			groupHeaders:[
				...
				{startColumnName:'rule01', numberOfColumns:1, titleText:'DB기준'},
                {startColumnName:'rule02', numberOfColumns:2, titleText:'시도대비'},
