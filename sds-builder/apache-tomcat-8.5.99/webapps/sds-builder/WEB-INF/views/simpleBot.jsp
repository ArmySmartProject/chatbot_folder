...
<script>
  ...
  $(document).ready(function () {
    ...
    if (env != 'prod') {
      ...
      getSimpleBotList({data: {userId: 'tjstmddus202@mindslab.ai', companyId: ''}});
    }
...
//         $('body').find(hrefId).before('<div class="lyr_bg"></div>');
    ...
    // 업로드 확인
    $('#scenario_upload .btn_submit').on('click', function () {
      handleLyrBoxClose();
      uploadExcel();
    });
    ...
    function uploadExcel() {
      ...
      $("#excelUploadForm input[name='simplebotId']").val(simplebotId);
...
//           $(".make_call").children('span').children('em').text(response['cnt']);
    ...
    $('#btn_delete_intent').on('click', function(){
      var delIntent = "utter"+ $('.actPop').parent().find('.sub_tit').text();
