var data = {userId: null, companyId: null, keyword: '', name: '', lang: -1, host: -1, simplebotId: -1};
...
var PAGES = {
  ...
  INTENTION_DETAIL : {
    title: '의도 상세',
    content: 'IntentionDetail',
    render: renderIntentionDetailPage,
  },
  INTENTION_ADD : {
    title: '의도 추가',
    content: 'IntentionAdd',
    render: renderIntentionAddPage,
  },
  TASK_DETAIL : {
    title: '태스크 상세',
    content: 'TaskDetail',
    render: renderTaskDetailPage,
  },
  TASK_ADD : {
		title: '태스크 추가',
		content: 'TaskAdd',
		render: renderTaskAddPage,
  },
  REPLACE_DIC : {
    title: '치환사전',
    content: 'ReplaceDic',
    active: 'replace',
    render: renderReplacePage,
  },
  INTENTION_ORDER : {
    title: '의도 순서',
    content: 'IntentionOrder',
    render: renderIntentOrderPage,
  }
}
...
window.addEventListener("message", function(e){
  selectScenario = createSearchSelect($('#scenario_list'));
  getVoiceBotList(e).then(function(){
    //시나리오 목록 선택
    selectScenario.select.on('change', function(){
      appliedAt = '';
      intentTableData = [];

      var getVal = selectScenario.getValue();
      if ( !getVal.length ) return;

      var value = getVal[0].value.split('_');
      data.host = value[0] === 'undefined'? -1 : value[0];
      data.simplebotId = value[1];
      data.lang = Number(value[2]);
      data.name = getVal[0].label;

      // 버튼 disabled 제거
      $('#vb_header .setting').removeAttr('disabled'); //헤더 셋팅 버튼
      $('#vb_gnb ul button').removeAttr('disabled'); //헤더 gnb 버튼

      getSimplebotById(data.simplebotId).then(function(){
        if ( scenario === null ) {
          $('#test_on').attr('disabled', '');
        } else {
        ...
        chatTestHtml += '<li class="blind">';
        chatTestHtml += '<p>START를 눌러 대화를</p>';
        chatTestHtml += '<p>시작해주세요.</p>';
        chatTestHtml += '<button type="button" class="btn_primary small btn_delete_blind" onclick="startChat();">START</button>';
        chatTestHtml += '</li>';

        $(".chat_talk").prepend(chatTestHtml);
...
function voiceBotMonitoring() {
    ...
    socket = io.connect('ws://10.50.1.19:51000', {'force new connection': true});
